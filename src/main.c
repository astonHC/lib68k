...
void PROC_FILE(char* FILENAME)
{
    ...
    OPCODE_BASE =  FIND_OPCODE(OPCODE_BASE, 16, "", "");
...
// THIS FUNCTION WILL BE USED AS THE CONJUNCTIVE AS SHOWN ABOVE

OPCODE* FIND_OPCODE(char* NAME, int SIZE, char* PROC, char* EA)
{
    OPCODE OPCODE_INPUT_TABLE[MAX_OP_BUFFER];

    // FOR EVERY SUBSEQUENT ITERATION AND OR ALLOCATABLE SPACE WITHIN THE TABLE
    // EVALUATE WHICH OPCODES CAN AND ARE STORED ACCORDINGLY

    for(OPCODE_BASE = OPCODE_INPUT_TABLE; OPCODE_BASE < &OPCODE_INPUT_TABLE; OPCODE_BASE++)
    {
        if(!OPCODE_BASE->NAME == 0)
        {
            return OPCODE_BASE;
        }
    }
