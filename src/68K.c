...
void M68K_SET_REGISTERS(struct CPU_68K* CPU_68K, int REGISTER, unsigned VALUE)
{
	switch (REGISTER)
	{
		...
		case M68K_USP: 
				if(FLAG_S) CPU_68K->USER_STACK = (U32*)M68K_MASK_OUT_ABOVE_32(VALUE);
					else
				CPU_68K->STACK_POINTER = (U32*)M68K_MASK_OUT_ABOVE_32(VALUE);
					return;

		case M68K_ISP:
				if(FLAG_S && !FLAG_M) CPU_68K->STACK_POINTER = (U32*)M68K_MASK_OUT_ABOVE_32(VALUE);
					else
				CPU_68K->INTERRUPT_SP = (U32*)M68K_MASK_OUT_ABOVE_32(VALUE);
					return;
